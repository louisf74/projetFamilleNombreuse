...
public class ConnexionActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    private boolean mayRequestContacts() {
        ...
        if (shouldShowRequestPermissionRationale(READ_CONTACTS)) {
            Snackbar.make(mPseudoView, R.string.permission_rationale, Snackbar.LENGTH_INDEFINITE)
    ...
    private void attemptLogin() {
        ...
        if (!TextUtils.isEmpty(password) && !isPasswordValid(password)) {
            mPasswordView.setError(getString(R.string.error_invalid_mdp));
    ...
    public void onLoadFinished(Loader<Cursor> cursorLoader, Cursor cursor) {
        ...
        while (!cursor.isAfterLast()) {
            pseudos.add(cursor.getString(ProfileQuery.ADDRESS));
            cursor.moveToNext();
        }

        addPseudosToAutoComplete(pseudos);
    }
    ...
    private void addPseudosToAutoComplete(List<String> pseudoAddressCollection) {
        //Create adapter to tell the AutoCompleteTextView what to show in its dropdown list.
        ArrayAdapter<String> adapter =
                new ArrayAdapter<>(ConnexionActivity.this,
                        android.R.layout.simple_dropdown_item_1line, pseudoAddressCollection);
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success) {
                ...
                mPasswordView.setError(getString(R.string.error_incorrect_mdp));
